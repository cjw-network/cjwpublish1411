...
/**
 *
 * ###JAC_PATCH_G_01_EZ_2014.11### host_uri default matching (+default browserlanguage match) + host (begins with matching)
 *                                 host : begins with
 * - kernel/classes/ezsiteaccess.php
 * - settings/site.ini
 *
 * ###JAC_PATCH_G_47_EZ_2014.11### CLEAR image-alias CACHE for project A also clear cache for all other projects
 *
 * ###JAC_PATCH_G_48_EZ_2014.11### multisite-hosting: CLEAR INI CACHE for project A also clear INI CACHE for all other projects on the same ez installation *
 *
            ...
            switch( $matchprobe )
            {
                    ...
                    if ( $match_type == 'map' )
                    {
                        if ( $ini->hasVariable( 'SiteAccessSettings', 'HostMatchMapItems' ) )
                        {
                            ...
                            foreach ( $matchMapItems as $matchMapItem )
                            {
                                ...
                                $matchMapAccess = $matchMapItem[1];
                                // ###JAC_PATCH_G_01_EZ_2014.11### host : begins with
                ...
                case 'host_uri':
                {
                    ...
                    if ( $ini->hasVariable( 'SiteAccessSettings', 'DefaultHostUriMatchMapItems' ) )
                    {
                        ...
                        // cjw_multiupload => upload =>$_SERVER['HTTP_ACCEPT_LANGUAGE'] not available
